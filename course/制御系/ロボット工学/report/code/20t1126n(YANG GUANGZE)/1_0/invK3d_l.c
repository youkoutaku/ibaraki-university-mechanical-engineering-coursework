...
//yang 3軸4リンクロボットアーム　逆運動学

// *** 先端位置 ***
void endPosition( len, tht, pos ) //始点・終点に関わる
	double *len;  //  Link length : L0, L1, L2, L3
	double *tht;  //  Joint theta :     J1, J2, J3
	double *pos;  // End position :  X,  Y,	 Z
{
	pos[X] = ( len[L2]*cos(tht[J2]) + len[L3]*cos(tht[J2]+tht[J3]) )*cos(tht[J1]);
	pos[Y] = ( len[L2]*cos(tht[J2]) + len[L3]*cos(tht[J2]+tht[J3]) )*sin(tht[J1]);
	pos[Z] = len[L1]+len[L2]*sin(tht[J2])+len[L3]*sin(tht[J2]+tht[J3]) ;
}
...
void jacobianMatrix( len, tht, Jaco )
	...
	MAT *Jaco;
{
	MATelem(*Jaco,1,1)= -( len[L2]*cos(tht[J2]) + len[L3]*cos(tht[J2]+tht[J3]) ) * sin(tht[J1]);
	MATelem(*Jaco,1,2)= -( len[L2]*sin(tht[J2]) + len[L3]*sin(tht[J2]+tht[J3]) ) * cos(tht[J1]);
	MATelem(*Jaco,1,3)= -len[L3] * sin(tht[J2]+tht[J3]) * cos(tht[J1]);
	MATelem(*Jaco,2,1)=  ( len[L2]*cos(tht[J2]) + len[L3]*cos(tht[J2]+tht[J3]) ) * cos(tht[J1]);
	MATelem(*Jaco,2,2)= -( len[L2]*sin(tht[J2]) + len[L3]*sin(tht[J2]+tht[J3]) )*sin(tht[J1]);
	MATelem(*Jaco,2,3)= -len[L3] * sin(tht[J2]+tht[J3]) * sin(tht[J1]);
	MATelem(*Jaco,3,1)= 0;
	MATelem(*Jaco,3,2)= len[L2]*cos(tht[J2]) + len[L3]*cos(tht[J2]+tht[J3]);
	MATelem(*Jaco,3,3)= len[L2] * cos(tht[J2]+tht[J3]);
}
