/*------------------------------------------------------------------*/
/*      		 			　　<ex6-6.c>	　　					*/
/*　　入れ子の構造体の配列を渡す関数のプログラム例（ポインタ表記）　*/
/*          ファイルから得点データを読み込み、成績評価と共に        */
/*    画面に出力する（成績の降順に並び替え ※ポインタをソートする） */
...
void sort_data(Prog2 *obj)
{
	...
	for(i=0;i<SNUM;i++){
		...
		printf("%-10d%-19s%10.1f%10.1f%10d%10.1f%10s\n",p[i]->ID,
        	p[i]->name, p[i]->sc.mini, p[i]->sc.rpt, p[i]->sc.final,
			p[i]->sc.score, p[i]->sc.grade);
	}
}
