...
{
	FILE 	*fp; 					//ファイルポインタ
	char 	fname[]="R12-20t1126n-walker.csv",	//読み込みファイル名
				ss[256],			//読み込み文字列格納用
...
{
  	switch (key) {
			case 'j':	case 'J':	//開始・時間加速
				glutTimerFunc(TIME_INTERVAL, timer, 0);//TIME_INTERVAL(ミリ秒)後にコールバック関数timerを実行
				break;
			case 'g':	case 'G':	case '\033':  // '\033' は ESC の ASCII コード 
				exit(0);
			case 'w':	case 'W':	//平行運動（上）
				move = 'W';
				break;
			case 's':	case 'S':	//平行移動（下）
				move = 'S';
				break;
			case 'a':	case 'A':	//平行移動（左）
				move = 'A';
				break;
			case 'd':	case 'D':	//平行移動（右）
				move = 'D' ;
				break;
			case 'y':	case 'Y':	//斜め移動（左上）
				move = 'Y';
				break;
			case 'i':	case 'I':	//斜め移動（右上）
				move = 'I';
				break;
			case 'h':	case 'H':	//斜め移動（左下）
				move = 'H';
				break;
			case 'k':	case 'K':	//斜め移動（右下）
				move = 'K';
				break;
			case 'f':	case 'F':	//一時移動停止
				move = 'F';
				break;
			case 'q':	case 'Q':	//拡大
				move = 'Q';
