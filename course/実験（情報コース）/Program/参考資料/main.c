...
main( void )
{
	...
	traj_tracking( -40.0*1.57f,  -100.0f,  5.0f );
	Feed_flag = STOP;
	#if 1
	save_data(3);  // データのセーブ
	#endif
...
//	#endif
	...
	//YANG 2022/12/21 車庫入れ後H8の処理

/*	//補足用直進走行
	Feed_flag = ENCO;
	// ( x_f[cm],  tht_f[deg],  t_f[s] );
	traj_tracking( 40.0*1.57f,  0.0f,  5.0f );
	Feed_flag = STOP;

	P2.DR.BIT.B5 = 1;//出力1として，センサを開始する．
*/

	//一回目直線運動
	while( 1 ){
		//100回の中に80一回以上だったら，通過する．
		int sum_B4;　//sum
		int d_B4  = 0; //距離
		int i_B4 = 0;
		for( i=0; i<100;i++){
			sum_B4 = sum_B4 + P2.DR.BIT.B4;
			motor(LW, 50*LD);
			motor(RW, 50*RD);
			d_B4 = d_B4 + 50;
		}
...
initialize_para( void )
{
	...
	// 0.0013725 [cm/pulse]
	Ki.W= 0.180f;    // ホイール間距離 [m]
	Ki.r= 0.021f;  // ホイール半径 [m]
