...
/*P4_B4
	...
	int i;
	for ( i=0; i<100; i++){
		printf("P2:4=%d\r", P4.DR.BIT.B4);
		sum_P4_B4 += P4.DR.BIT.B4;
	}
	printf("%d\r",sum_P4_B4);
*/	
...
/*	for ( i=0; i<500; i++){
	...
	// --- ライントレース ---
	// ----------------------
	for ( lap_flag=0 ; lap_flag<4 ; lap_flag++) //2周で終了
	{
...
//			sleep(20);
	...
	Feed_flag =ENCO;
	traj_tracking( 40.0*1.57f,  90.0f,  5.0f );
	Feed_flag = STOP;
	#if 1
	save_data(1);  // データのセーブ
	#endif
	...
	traj_tracking( -40.0*1.57f,  -105.0f,  5.0f );
	Feed_flag = STOP;
	#if 1
	save_data(3);  // データのセーブ
	#endif
...
*/
	//Feed_flag = ENCO;
	// ( x_f[cm],  tht_f[deg],  t_f[s] );
	/*traj_tracking( -10.0f,  0.0f,  2.0f );
	Feed_flag = STOP;
	#if 1
	save_data(4);  // データのセーブ
	#endif*/
	//YANG 2022/1/20 車庫入れ後H8の処理
	
	//補足用1
/*	Feed_flag = ENCO;
	 //( x_f[cm],  tht_f[deg],  t_f[s] );
	traj_tracking( 60.0f,  0.0f,  5.0f );
	Feed_flag = STOP;*/
...
//一回目直線運動
	...
	int i_P2_B4;
	while(1){
		for (i_P2_B4=0;i_P2_B4<100*0.5;i_P2_B4++){
			sum_P2_B4 = sum_P2_B4 + P2.DR.BIT.B4;
			motor(LW, 150*LD);
			motor(RW, 150*RD);
			printf("B4=%d\r", P2.DR.BIT.B4);	
		}
		printf("%d\r", sum_P2_B4);
		if(sum_P2_B4 >= 60*0.5 ){
				break;
		}
	}
...
/*	int i_P2_B5_r = 0;
	int sum_r =0;
	while(1){
		for( i_P2_B5_r=0; i_P2_B5_r<10 ;i_P2_B5_r++ ){
			sum_r = sum_r + P2.DR.BIT.B5;
			motor(LW, 0*LD);
			motor(RW, 100*RD);
		}
		if( sum_r >6 ){
	...
	int i_P2_B5;
	while(1){
		for (i_P2_B5=0;i_P2_B5<100;i_P2_B5++){
			sum_P2_B5 = sum_P2_B5 + P2.DR.BIT.B5;
			motor(LW, 150*LD);
			motor(RW, 150*RD);
		}
		if(sum_P2_B5 >= 60){
...
initialize_para( void )
{
	...
	// --- ハードウェアパラメータをセット ---
	//(YANG):車輪の半径は0.0185, 車輪間の距離は，設計図によりおよそ0.130, この後に再びチェック必要
	//11/09: 距離:0.18 半径:0.021
	// ホイールが一回転回るとき (Ki.r)*2*PI [cm] <-> (Ct.Tr)*(128*4) [pulse] 
	// 0.0013725 [cm/pulse]
	Ki.W= 0.1965f;    // ホイール間距離 [m]
	Ki.r= 0.02115f;  // ホイール半径 [m]
	Ct.WPulse2Rad= 2.0f*PI/512.0f;  // エンコーダのパルスを回転角[rad]に変換, 512=128[pulse]*4[逓倍]
	Ct.Tr= 29.0f;    // モータ減速比

	// --- ロボットのゲインをセット ---
	Ct.jnt[LW].gain.kp= 80.0f;
	Ct.jnt[LW].gain.kd= 80.0f*RsampTime;

	Ct.jnt[RW].gain.kp= 80.0f;
	Ct.jnt[RW].gain.kd= 80.0f*RsampTime;
